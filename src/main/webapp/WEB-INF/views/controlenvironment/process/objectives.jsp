<%@page contentType="text/html" pageEncoding="UTF-8" %>
<% session.setAttribute("sideBar", 12);%>
<%@include file="/WEB-INF/views/template/include.jsp" %>
<!DOCTYPE html>
<html>
<head>
    <title>Datos generales del Proceso</title>
    <%@include file="/WEB-INF/views/template/styles.jsp" %>
</head>

<body class="fixed-header">

<%@include file="/WEB-INF/views/template/sideBar.jsp" %>
<!-- START PAGE-CONTAINER -->
<div class="page-container">
    <!-- START HEADER -->
    <%@include file="/WEB-INF/views/template/navBar.jsp" %>
    <!-- END HEADER -->
    <div class="page-content-wrapper">
        <div class="content">
            <div class="register-container full-height full-width sm-p-t-30"
                 style="padding-left: 40px; padding-right: 40px;">
                <div class="container-sm-height full-height">
                    <div class="row row-sm-height">
                        <div class="col-sm-12 col-sm-height col-middle">
                            <c:if test='${objectProcessSelected.responsible.id==userLogged.id}'>
                                <h3>Complete los datos generales del Proceso</h3>
                                <p>
                                    <small>
                                        Complete los datos restantes del proceso. Los objetivos, misión y visión son la
                                        base
                                        de cualquier proceso.
                                        Es importante tenerlos definidos antes de realizar cualquier operación en el
                                        resto
                                        de los componentes del sistema.
                                    </small>
                                </p>
                            </c:if>
                            <c:if test='${objectProcessSelected.responsible.id!=userLogged.id}'>
                                <h3>Datos generales del Proceso</h3>
                            </c:if>
                            <form novalidate="novalidate" id="form-register" class="p-t-15" role="form" action="save"
                                  method="POST">
                                <div class="row">
                                    <div class="col-sm-12">
                                        <div class="col-sm-12">
                                            <h5 class="m-b-0">Nombre</h5>
                                            <p class="m-l-5">${objectProcessSelected.name}</p>
                                        </div>
                                    </div>
                                </div>
                                <div class="row">
                                    <div class="col-sm-12">
                                        <div class="col-sm-12">
                                            <h5 class="m-b-0">Descripción</h5>
                                            <p class="m-l-5">${objectProcessSelected.description}</p>
                                        </div>
                                    </div>
                                </div>
                                <div class="row">
                                    <div class="col-sm-12">
                                        <c:if test='${objectProcessSelected.responsible.id==userLogged.id}'>
                                            <security:authorize access="hasRole('ROLE_PROCESS_SUPERVISORY')">
                                                <div class="form-group form-group-default">
                                                    <label>Visión</label>
                                             <textarea id="vision" name="vision"
                                                       placeholder="Visión general del proceso" rows="5"
                                                       class="full-width"
                                                       style="border: 0px;"
                                                       onfocus="$(this).attr('class','full-width focused')"
                                                       onblur="$(this).attr('class','full-width')">${objectProcessSelected.vision}</textarea>
                                                </div>
                                            </security:authorize>
                                        </c:if>
                                        <c:if test='${objectProcessSelected.responsible.id!=userLogged.id}'>
                                            <h5 class="m-b-0">Visión</h5>
                                            <p class="m-l-5">${objectProcessSelected.vision}</p>
                                        </c:if>

                                    </div>
                                </div>
                                <div class="row">
                                    <div class="col-sm-12">
                                        <c:if test='${objectProcessSelected.responsible.id==userLogged.id}'>
                                            <security:authorize access="hasRole('ROLE_PROCESS_SUPERVISORY')">
                                                <div class="form-group form-group-default">
                                                    <label>Misión</label>
                                            <textarea id="mission" name="mission"
                                                      placeholder="Misión del proceso" rows="5" class="full-width"
                                                      style="border: 0px;"
                                                      onfocus="$(this).attr('class','full-width focused')"
                                                      onblur="$(this).attr('class','full-width')">${objectProcessSelected.mission}</textarea>
                                                </div>
                                            </security:authorize>
                                        </c:if>
                                        <c:if test='${objectProcessSelected.responsible.id!=userLogged.id}'>
                                            <h5 class="m-b-0">Misión</h5>
                                            <p class="m-l-5">${objectProcessSelected.mission}</p>
                                        </c:if>
                                    </div>
                                </div>
                                <div class="panel panel-default">
                                    <div class="panel-heading">
                                        <div class="panel-title">Objetivos
                                        </div>
                                        <div class="pull-right">
                                            <c:if test='${objectProcessSelected.responsible.id==userLogged.id}'>
                                                <div class="col-xs-7">
                                                    <input type="text" placeholder="Buscar"
                                                           class="form-control pull-right"
                                                           id="search-table">
                                                </div>
                                                <div class="btn-group pull-right col-xs-5">
                                                    <button id="showmodalAdd" class="btn btn-primary" type="button">
                                                        Nuevo
                                                        Objetivo
                                                    </button>
                                                </div>
                                            </c:if>
                                            <c:if test='${objectProcessSelected.responsible.id!=userLogged.id}'>
                                                <div class="col-xs-11">
                                                    <input type="text" placeholder="Buscar"
                                                           class="form-control pull-right"
                                                           id="search-table">
                                                </div>
                                            </c:if>
                                        </div>
                                        <div class="clearfix"></div>
                                    </div>
                                    <div class="panel-body">
                                        <div id="tableWithSearch_wrapper"
                                             class="dataTables_wrapper form-inline no-footer">
                                            <div class="table-responsive">
                                                <table id="tableWithSearch"
                                                       class="table table-hover demo-table-search dataTable no-footer"
                                                       role="grid" aria-describedby="tableWithSearch_info">
                                                    <thead>
                                                    <tr role="row">
                                                        <th class="sorting_asc" tabindex="0"
                                                            aria-controls="tableWithSearch" rowspan="1"
                                                            colspan="1" style="width: 180px;" aria-sort="ascending">
                                                            Nombre
                                                        </th>
                                                        <th class="sorting" tabindex="0" aria-controls="tableWithSearch"
                                                            rowspan="1"
                                                            colspan="1" style="width: 250px;">Descripción
                                                        </th>
                                                        <c:if test='${objectProcessSelected.responsible.id==userLogged.id}'>
                                                            <th tabindex="0" aria-controls="tableWithSearch" rowspan="1"
                                                                colspan="1"
                                                                style="width: 80px;">Acciones
                                                            </th>
                                                        </c:if>
                                                    </tr>
                                                    </thead>
                                                    <tbody>
                                                    <c:forEach var="objective" items="${objectProcessSelected.objectives}">
                                                        <tr role="row" class="odd">

                                                            <td class="v-align-middle semi-bold sorting_1">
                                                                <p>${objective.objective}</p>
                                                            </td>
                                                            <td class="v-align-middle">
                                                                <p>${objective.description}</p>
                                                            </td>
                                                            <c:if test='${objectProcessSelected.responsible.id==userLogged.id}'>
                                                                <td class="v-align-middle">
                                                                    <div class="row">
                                                                        <div class="btn-group b-grey b-l b-r p-l-15 p-r-15">
                                                                            <button class="btn btn-success"
                                                                                    type="button"
                                                                                    onclick="fillObjectiveModal(this)">
                                                                                <i
                                                                                        class="fa fa-pencil"></i>
                                                                            </button>
                                                                            <button class="btn btn-success"
                                                                                    type="button"
                                                                                    onclick="deleteObjective(this)"><i
                                                                                    class="fa fa-trash-o"></i>
                                                                            </button>
                                                                        </div>
                                                                    </div>
                                                                    <input type="text" hidden="true"
                                                                           value="${objective.id}"/>
                                                                </td>
                                                            </c:if>
                                                        </tr>
                                                    </c:forEach>
                                                    </tbody>
                                                </table>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                <c:if test='${objectProcessSelected.responsible.id==userLogged.id}'>
                                    <button class="btn btn-primary btn-cons" type="button"
                                            onclick="completeProcessData();">
                                        Guardar Cambios
                                    </button>
                                </c:if>
                            </form>
                        </div>
                    </div>
                </div>
                <br/>
            </div>
            <!-- MODAL OBJECTIVE ADD UP  -->
            <div class="modal fade stick-up" id="addNewAppModal" tabindex="-1" role="dialog"
                 aria-labelledby="addNewAppModal" aria-hidden="true">
                <div class="modal-dialog">
                    <div class="modal-content">
                        <div class="modal-header clearfix ">
                            <button type="button" class="close" data-dismiss="modal" aria-hidden="true"
                                    onclick="cleanGuideModal();"><i
                                    class="pg-close fs-14"></i>
                            </button>
                            <h4 class="p-b-5"><span class="semi-bold">Nuevo</span> Objetivo
                                <div class="pull-right hidden" id="progress-bar-add">
                                    <div class="col-sm-1 text-center center-scale">
                                        <div class="progress-circle-indeterminate m-t-0">
                                        </div>
                                    </div>
                                </div>
                            </h4>
                            <div id="modalMessage"></div>
                        </div>
                        <div class="modal-body">
                            <form role="form" method="POST">
                                <div class="row">
                                    <div class="col-sm-12">
                                        <div class="form-group form-group-default">
                                            <label>Nombre</label>
                                            <input id="name" name="name" type="text" class="form-control"
                                                   placeholder="Nombre o título del objetivo" required="true">
                                        </div>
                                    </div>
                                </div>
                                <div class="row">
                                    <div class="col-sm-12">
                                        <div class="form-group form-group-default">
                                            <label>Descripción</label>
                                    <textarea id="description" name="description"
                                              placeholder="Descripción del objetivo (Opcional)" rows="2"
                                              class="full-width"
                                              style="border: 0px;" onfocus="$(this).attr('class','full-width focused')"
                                              onblur="$(this).attr('class','full-width')"></textarea>
                                        </div>
                                    </div>
                                </div>
                                <button id="add-objective" type="submit" class="btn btn-primary  btn-cons pull-right"
                                        onclick="if (validateObjectiveModal()) {
                               addObjective();}return false;">Aceptar
                                </button>
                            </form>
                        </div>
                        <div class="modal-footer">
                        </div>
                    </div>
                    <!-- /.modal-content -->
                </div>
                <!-- /.modal-dialog -->
            </div>
            <!-- END MODAL OBJECTIVE ADD UP  -->

            <!-- MODAL OBJECTIVE EDIT UP  -->
            <div class="modal fade stick-up" id="editAppModalObjective" tabindex="-1" role="dialog"
                 aria-labelledby="editAppModalObjective" aria-hidden="true">
                <div class="modal-dialog">
                    <div class="modal-content">
                        <div class="modal-header clearfix ">
                            <button type="button" class="close" data-dismiss="modal" aria-hidden="true"><i
                                    class="pg-close fs-14"></i>
                            </button>
                            <h4 class="p-b-5"><span class="semi-bold">Modificar</span> Objetivo
                                <div class="pull-right hidden" id="progress-bar-edit">
                                    <div class="col-sm-1 text-center center-scale">
                                        <div class="progress-circle-indeterminate m-t-0" data-color="primary">
                                        </div>
                                    </div>
                                </div>
                            </h4>
                            <div id="editModalMessage"></div>
                        </div>
                        <div class="modal-body">
                            <form role="form" method="POST" action="">
                                <div class="row">
                                    <div class="col-sm-12">
                                        <div class="form-group form-group-default">
                                            <label>Nombre</label>
                                            <input id="editName" name="editName" type="text" class="form-control"
                                                   placeholder="Nombre o título del cuestionario" required="true">
                                        </div>
                                    </div>
                                </div>
                                <div class="row">
                                    <div class="col-sm-12">
                                        <div class="form-group form-group-default">
                                            <label>Descripción</label>
                                    <textarea id="editDescription" name="editDescription"
                                              placeholder="Descripción del cuestionario (Opcional)" rows="2"
                                              class="full-width"
                                              style="border: 0px;" onfocus="$(this).attr('class','full-width focused')"
                                              onblur="$(this).attr('class','full-width')"></textarea>
                                        </div>
                                    </div>
                                </div>
                                <button id="edit-objective" type="button" class="btn btn-primary  btn-cons pull-right">
                                    Aceptar
                                </button>
                            </form>
                        </div>
                        <div class="modal-footer">
                        </div>
                    </div>
                    <!-- /.modal-content -->
                </div>
                <!-- /.modal-dialog -->
            </div>
            <!-- END MODAL OBJECTIVE EDIT UP  -->

        </div>


        <!-- BEGIN COPYRIGHT -->
        <%@include file="/WEB-INF/views/template/copyright.jsp" %>
        <!-- END COPYRIGHT -->
    </div>
    <!-- BEGIN JS -->
    <%@include file="/WEB-INF/views/template/script.jsp" %>
    <script src="${pageContext.request.contextPath}/resources/js/controlenvironment/process/process.js"
            type="text/javascript"></script>

    <!-- BEGIN ID -->
    <input id="idObjective" type="text" hidden="true" value=""/>
    <!-- END ID -->
</div>
</body>
</html>
